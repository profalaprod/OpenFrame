...
export function useVideoPlayer({
  ...
  useEffect(() => {
    ...
    const isDirect = activeProviderId === 'direct';

    if (isYoutube && !isApiLoaded) return;
    if (!isYoutube && !isBunny && !isDirect) return;
  ...
  const handlePlayPause = useCallback(() => {
    if (activeProviderId === 'bunny' || activeProviderId === 'direct') {
      const videoEl = videoRef.current;
      if (!videoEl) return;
  ...
  }, [activeProviderId, isPlaying, playerRef, videoRef]);
  ...
  const handleSeekToTimestamp = useCallback(
    (timestamp: number, annotation?: string | null, options?: { pauseAfterSeek?: boolean }) => {
      ...
      if (
        (activeProviderId === 'bunny' || activeProviderId === 'direct') &&
        videoRef.current
      ) {
    ...
    [activeProviderId, isPlaying, playerRef, setViewingAnnotation, videoRef]
  );
  ...
  const handleMuteToggle = useCallback(() => {
    ...
    setIsMuted(!isMuted);
  }, [activeProviderId, isMuted, playerRef, videoRef]);
  ...
  const handleSpeedChange = useCallback(
    ...
    [activeProviderId, playerRef, videoRef]
  );
